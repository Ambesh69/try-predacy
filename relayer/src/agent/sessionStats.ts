...
export class SessionStats {
  ...
  recordSnapshot(sessionLabel: string, handleIdHex: string, snap: GameState): SnapshotResult {
    ...
    if (becamePostflop && s.currentHandPlayers.length === 0) {
      s.currentHandIdx += 1;
      // Cap matches handLevelMarketsFor's 2-player cap so we seed and
      // settle exactly the same set per hand (~2 SOL/hr at 50 hands/hr).
      s.currentHandPlayers = snap.inHandPlayers.slice(0, 2);
      result.handStart = {
