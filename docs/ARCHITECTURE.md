...
## 12. Integrations — what we use and why

Predacy is built on a set of Solana-native primitives that each solve a specific architectural problem. Surface integrations are listed elsewhere; the choices below shape the protocol.

| Integration          | What it solves for Predacy                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Encrypt (REFHE)**  | Batch clearing under FHE so the relayer never sees an order's side, limit, or amount during the 30s sealed-bid window. The `programs/predacy-fhe/` Anchor program is the actual clearing primitive — a 4-side uniform-price algorithm expressed as arithmetic-only homomorphic ops (~80 per batch), 13 encrypted inputs → 4 encrypted aggregates. Plaintext clearing collapses to a relayer-trust model. |
| **Ika dWallets**     | 2PC-MPC custody that signs on both Curve25519 (Solana) and Secp256k1 (Polygon) from the same logical key. Lets Tier-2 residuals route to Polymarket's CLOB without bridges, wrapped assets, or operational float. Without it, cross-chain liquidity collapses to "bridge USDC and pray."                                                                                                                |
| **Umbra Privacy**    | Frontend funding-flow privacy. Order capital moves through an Umbra shielded pool to an ephemeral signer so the main wallet ↔ order link is broken before the order even reaches the batch. Without it, on-chain accounting still leaks the main wallet through the funding tx graph.                                                                                                                  |
| **RPC Fast**         | Sub-100ms event delivery via Yellowstone gRPC — batch open / close / lock / settle / claim events stream to the frontend with SSE fanout. Replaces poll-driven UI with push-driven UI. Also: SWQoS priority routing for the relayer's settlement txs so batches don't dropped under network load.                                                                                                       |
| **Privy**            | Solana-only wallet auth (SIWS), no email / social. The only identifier Privy holds is the Solana pubkey; account creation is a single tap. Bridges the "embed a wallet experience" UX gap without compromising the anonymous-account property.                                                                                                                                                         |
| **Polymarket**       | The deep CLOB book Predacy graduates *into*, not competes with. Our routing model assumes Polymarket is the deep tail — Tier 0/1 handles our short-duration, thin-maker markets, Tier 2 bridges residual flow over Ika to take the existing $3B+ liquidity.                                                                                                                                            |

---

## 13. Devnet deployment status
...
| Polymarket CLOB routing      | **Routing pipeline live on devnet; Polygon-side execution gated on Ika mainnet.** Relayer's `pairMatcher.ts` splits every settled batch into Path A (complete-set matchable pairs) and Path B (residuals needing CLOB). `polymarketRouter.ts` produces a structured routing manifest with per-user fill receipts, exposed via `GET /settlement-stats`. The Solana-side accounting, capital reservation, and manifest generation are production-grade; the cross-chain leg unblocks when Ika mainnet + a hosted CLOB on Polygon Amoy testnet land. Unit-tested 16/16 across balanced, imbalanced, sell-absorbed, and sell-only batches. |
| LP market for Polygon USDC   | **LP market simulator live on devnet with full routing + accounting.** `relayer/src/lpRegistry.ts` implements an LP registry with three demo LPs (Boutique Capital @10bps, Genesis LP @15bps, Swift Liquidity @20bps; $160k aspirational capacity). `polymarketRouter.ts` picks the cheapest LP with enough capacity per batch (lowest feeBps, tie-break on deeper pool), reserves capital up front, logs the quote. Each settled batch records `liquidityProvider: {lpId, displayName, usdcAmount, feeUsdc, feeBps}` on its settlement stats. Endpoints: `GET /lps` (list), `POST /lps/register` (add), `POST /lps/:id/active` (toggle). Unit-tested 16/16 across cheapest-wins, capacity-fallback, fee-tie-break, inactive-skip, and reserve/release accounting. Polygon-side capital movement plugs in once the Ika cross-chain swap (Todo #19) lands. |
| FHE batch clearing (strict mode) | **Two-layer strict-mode shipped end-to-end on devnet.** (1) Relayer layer — `relayer/src/fheBackend.ts` + `encryptedClearing.ts` — re-expresses the clearing algorithm as an arithmetic-only homomorphic circuit. The transparent ciphertext backend runs the full path today and cross-checks bit-identical against plaintext (41/41 unit tests); `PRIVACY_MODE=strict` proven end-to-end on devnet (batch 12 settled via strict path, claim tx `5cWtbufit…`). (2) **On-chain layer — Encrypt REFHE Anchor program live on Solana devnet with the production clearing algorithm.** `programs/predacy-fhe/` uses Encrypt's Rust crates (`encrypt-anchor`, `encrypt-solana-dsl`, `encrypt-types` from https://github.com/dwallet-labs/encrypt-pre-alpha). Defines `#[encrypt_fn] fn settle_batch_graph(...)` — the complete four-side uniform-price clearing algorithm for 4-order batches, expressed as arithmetic-only homomorphic ops (`==`, `>=`, `<=`, `+`, `*` — all mapped to Encrypt DSL codes, auto-promoted to scalar variants when one operand is a plaintext literal). Takes 13 encrypted inputs (4 orders × {side, limit, amount} + clearing price), returns 4 encrypted filled-volume aggregates. ~80 HE ops per batch. Compiled to a 201KB SBF binary via `anchor build`; deployed to Solana devnet at `59ZxSvmRrzCWo4vFjUrdp8sZDCvW2yGU2MGG5EqesLQn` (latest redeploy tx `43qDbXVXhi…`). CPI target: Encrypt's coprocessor program `4ebfzWdKnrnGseuQpezXdG8yCdHqwQ1SSBHD3bWArND8`. Encrypt's devnet executor runs the real graph through plaintext-simulation by design today; when Alpha 1 ships on-chain FHE, the same bytes execute confidentially with zero program change. Scoped to 4 orders (vs. Groth16 path's 8) because 4-order input+output ciphertext accounts (17 total) fit in a standard Solana tx without address lookup tables — scaling to 8 is additive (loop unroll + ALT), not new logic. **End-to-end pipeline status: full FHE batch clearing live, byte-correct decrypted output on Solana devnet.**
