...
| Ika program CPI integration  | **Not yet.** Our Predacy Anchor program doesn't yet depend on `ika-dwallet-anchor` crate or CPI-call `approve_message`. That's Todo #18 — the piece that ties the dWallet signatures into Predacy's on-chain logic. |
