...
| Ika program CPI integration  | **Built + relayer-side transfer ready. Pending devnet redeploy.** Program compiles with the new `approve_ika_message` instruction; relayer has `POST /ika/transfer-authority` (and `transferDWalletToPredacy()` method) to move a dWallet's authority from the DKG payer to Predacy's CPI PDA. Deploy needs ~3.7 SOL devnet to cover the new 531KB buffer allocation. Once deployed: call `/ika/transfer-authority` → call predacy.approve_ika_message (client) → call `/ika/sign` → real signature. |
