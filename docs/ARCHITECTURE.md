...
| Ika dWallet (Pre-Alpha, relayer-side) | **Live on devnet: DKG + Presign + Sign (BCS correct)**. DKG succeeds for Curve25519/Ed25519 (Solana-compat, PDA `FFu8UwCJ…`) and Secp256k1/ECDSA (Polygon-compat, PDA `8NJ9YfFP…`). Presign succeeds for both. Sign request is BCS-correct end-to-end — Ika's server parses it fully and enforces the architectural contract: **Sign requires a `MessageApproval` PDA to pre-exist on-chain, created via CPI from our Predacy program calling `approve_message` on Ika's program.** That CPI integration is Todo #20. Endpoints `/ika/dwallet`, `/ika/presign`, `/ika/sign` all live (sign returns "MessageApproval PDA not found" until CPI ships). Pre-Alpha caveat: single mock signer (not real MPC) — real 2PC-MPC ships with Ika Alpha 1. |
| Ika program CPI integration  | **Architecturally complete on devnet. Gated on Ika Alpha 1 for real signatures** (2026-04-24). Program redeployed with `approve_ika_message` (tx `hGR7XVFM…`, program ID `Bb3z…`). Authority transfer works. `POST /ika/approve-and-sign` chains approve → presign → sign; MessageApproval PDA (e.g. `5G1pfCdb…`) lands on-chain, Ika-owned. Final Sign returns `"no key for dwallet <payer_pubkey>"` regardless of configuration — Pre-Alpha's mock signer never registers a signing key for any payer (verified by running a fresh-keypair exact-match reproduction — see `relayer/scripts/test-ika-fresh.ts`). Alpha 1 replaces the mock with real 2PC-MPC and closes this gap without any code changes on our side. |
