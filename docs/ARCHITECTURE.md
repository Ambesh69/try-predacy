...
| Ika dWallet (Pre-Alpha, relayer-side) | **Scaffolded, awaiting live test.** Relayer has `IkaManager` (`relayer/src/ika.ts`) with DKG + presign flows over Ika's gRPC signing service. Endpoints `POST /ika/dwallet` + `POST /ika/presign` live. Activates when `IKA_ENABLED=true`. Supports Curve25519/EdDSA (Solana) and Secp256k1/ECDSA (Polygon) curves. Pre-Alpha caveat: single mock signer (not real MPC) — real 2PC-MPC ships with Ika Alpha 1. |
