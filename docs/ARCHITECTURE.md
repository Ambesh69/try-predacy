...
| Ika program CPI integration  | **Live on devnet. Full path proven end-to-end up to mock-signer limit** (2026-04-24). Program redeployed with `approve_ika_message` (tx `hGR7XVFM…`, existing program ID `Bb3z…`). Authority transfer works (verified for both dWallets). `POST /ika/approve-and-sign` chains approve → presign → sign in one call; MessageApproval PDA (e.g. `5G1pfCdb…`) lands on-chain owned by Ika program. Final Sign returns `"no key for dwallet"` — **Pre-Alpha mock signer limitation**: placeholder-DKG'd dWallets have no registered signing key server-side. Real MPC in Alpha 1 will close that gap; our integration plumbing is complete. |
