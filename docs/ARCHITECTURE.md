...
| Ika dWallet (Pre-Alpha, relayer-side) | **Live on devnet, DKG + Presign both working** (2026-04-23 verified). DKG succeeds for Curve25519/Ed25519 (Solana-compat, PDA `FFu8UwCJ…`) and Secp256k1/ECDSA (Polygon-compat, PDA `8NJ9YfFP…`). Presign succeeds for both curves. Attestations persisted to `ika-dwallets.json` (gitignored, 0600 perms). Endpoints `POST /ika/dwallet` (idempotent) and `POST /ika/presign` live. Pre-Alpha caveat: single mock signer (not real MPC) — real 2PC-MPC ships with Ika Alpha 1. |
