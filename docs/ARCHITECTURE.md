...
| Ika program CPI integration  | **Built, not yet deployed.** `programs/predacy/src/instructions/approve_ika_message.rs` now CPI-calls `approve_message` on the Ika program. `anchor build` produces a 531KB `.so` with 14 instructions (added `approve_ika_message`). IDL includes the new instruction. Ready to `anchor deploy --provider.cluster devnet` when desired. Once deployed, `/ika/sign` will return real signatures end-to-end. |
