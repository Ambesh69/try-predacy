...
| Unwrap destination picker    | **Not yet in UI.** Claimed tokens land in ephemeral ATA; "Move to address" action (next todo) will let users pull them out to any destination via Umbra. Payout-directly-into-shielded-UTXO (skipping the ATA entirely) is a further hardening step. |
