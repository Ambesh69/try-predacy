...
| Ika dWallet (Pre-Alpha, relayer-side) | **Live on devnet** (2026-04-23 verified). DKG succeeded for both Curve25519/Ed25519 (Solana-compat, PDA `FFu8UwCJ…`) and Secp256k1/ECDSA (Polygon-compat, PDA `8NJ9YfFP…`). Attestations persisted to `ika-dwallets.json` (gitignored, 0600 perms). Endpoint `POST /ika/dwallet` idempotent. Presign endpoint still fails with BCS re-serialization issue — follow-up work. Pre-Alpha caveat: single mock signer (not real MPC) — real 2PC-MPC ships with Ika Alpha 1. |
