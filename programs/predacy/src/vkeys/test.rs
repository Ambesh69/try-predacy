//! Canonical test verifying key from groth16-solana@0.2.0's own test suite.
//! Copied verbatim from the crate's `src/groth16.rs` #[cfg(test)] mod. Used
//! by `verify_test_vectors` to discriminate between
//!   (a) our encoding has a subtle bug snarkjs/ark_bn254 tolerates but the
//!       Solana alt_bn128_pairing syscall doesn't, vs
//!   (b) the devnet syscall rejects well-formed proofs that ark_bn254 accepts
//! If this test-vector verifies on-chain, (b) is false → our proofs have an
//! issue. If it fails on-chain too, (b) is the real story.
use groth16_solana::groth16::Groth16Verifyingkey;

pub const TEST_VK: Groth16Verifyingkey = Groth16Verifyingkey {
    nr_pubinputs: 10,
...
/// 9 public inputs matching TEST_VK. Byte-for-byte from the crate's test.
pub const TEST_PUBLIC_INPUTS: [[u8; 32]; 9] = [
    [
...
/// proof_a: first 64 bytes of PROOF[0..64], then NEGATED (y → p-y) so it
/// matches what `Groth16Verifier::new` expects. Computed via ark_bn254 in
/// /tmp/g16-repro/src/bin/compute_neg.rs to avoid pulling ark as a program
/// dep. See /tmp/g16-repro/ output.
pub const TEST_PROOF_A: [u8; 64] = [
...
/// proof_b: PROOF[64..192] — passed directly per crate test.
pub const TEST_PROOF_B: [u8; 128] = [
...
/// proof_c: PROOF[192..256] — passed directly per crate test.
pub const TEST_PROOF_C: [u8; 64] = [
